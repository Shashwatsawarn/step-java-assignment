...
public class W5P5PlacementShortlisting {
    static class Candidate implements Comparable<Candidate> {
        ...
        double compositeScore() { return cgpa * 10 + codingScore / 2.0; }
    ...
    static String shortlistAndRank(Candidate[] candidates) {
        Candidate[] shortlisted = new Candidate[candidates.length]; int count = 0;
        for (Candidate candidate : candidates)
            if (isEligible(candidate.cgpa) || isEligible(candidate.cgpa, candidate.codingScore)) shortlisted[count++] = candidate;
        shortlisted = Arrays.copyOf(shortlisted, count); Arrays.sort(shortlisted);
        StringBuilder answer = new StringBuilder();
        for (int i = 0; i < shortlisted.length; i++) {
            if (i > 0) answer.append(" | ");
            answer.append(i + 1).append(". ").append(shortlisted[i].name).append(" (").append(shortlisted[i].compositeScore()).append(")");
        }
